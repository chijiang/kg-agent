...
def upgrade() -> None:
    ...
    if not column_exists('users', 'approval_note'):
        op.add_column('users', sa.Column('approval_note', sa.String(length=500), nullable=True))
    ...
    if not column_exists('users', 'is_password_changed'):
        op.add_column('users', sa.Column('is_password_changed', sa.Boolean(), nullable=False, server_default=sa.false()))
    ...
    for page in pages:
        op.execute(
            sa.text("INSERT INTO role_page_permissions (role_id, page_id) VALUES (:role_id, :page_id)")
